...
func setApiRouter(r *resource) {
    ...
    // order
    orders := r.mux.Group("/api/order", core.WrapAuthHandler(r.interceptors.CheckLogin), r.interceptors.CheckSignature(), r.interceptors.CheckRBAC())
    {
        orders.POST("/create", orderHandler.Create())
        orders.POST("/cancel", orderHandler.Cancel())
        orders.GET("/detail/:id", core.AliasForRecordMetrics("/api/order/detail"), orderHandler.Detail())
    }
}
